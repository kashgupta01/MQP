...
with open("batch_output_6.jsonl", "r") as f:
    for i, line in enumerate(f, start=1):
        ...
        # try regex parse
        match = re.search(
            r"Complex Function:\s*(.*?)\s*Organism:\s*(.*?)\s*Proteins:\s*(.*?)\s*Genes:\s*(.*?)\s*Self Confidence Score:\s*(.*)",
            content,
            re.DOTALL
        )

        if match:
            complex_function, organism, proteins, genes, self_confidence_score = match.groups()
            data.append([
                complex_name.strip(),
                technique.strip(),
                llm_name,
...
df = pd.DataFrame(data, columns=["Complex", "Technique", "LLM", "Complex Function", "Organism", "Proteins", "Genes", "Self Confidence Score"])
